...
/**
 * Automatically loads weekly steps for the signed-in user and plots a Plotly bar chart.
 * Works with multiple response shapes:
 *  - [{date:"YYYY-MM-DD", steps:1234}, ...]
 *  - { days: [{date, steps}, ...] }
 *  - { payload: { days: [...] } }
 *  - { data: [...] }
 *
 * If your API returns a different shape, the console will show the raw response for quick debugging.
 */
export default function WeeklyStepsAutoPlot({ apiBase = "http://localhost:5000" }) {
    const [user, setUser] = useState(null);
    const [stepsData, setStepsData] = useState([]); // normalized array {date, steps}
    ...
    // Fetch whenever user becomes available
    useEffect(() => {
        if (!user) {
            setStepsData([]);
            return;
        ...
        async function fetchWeekly() {
            ...
            try {
                ...
                // If not OK, try to give clear message
                if (!res.ok) {
                ...
                console.debug("Weekly steps raw response:", raw); // <--- check this in console
                ...
                if (mounted) setStepsData(normalized);
            } catch (err) {
